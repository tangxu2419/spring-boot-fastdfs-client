...
public class FieldMetaData {
    ...
    public byte[] toByte(Object bean, Charset charset)
        ...
        if (isDynamicField()) {
            ...
            // 如果是动态属性
            return ProtoPackageUtil.objString2Byte((String) value, max, charset);
        } else if (long.class.equals(field.getType())) {
            return ProtoPackageUtil.long2buff((long) value);
        } else if (int.class.equals(field.getType())) {
            return ProtoPackageUtil.long2buff((int) value);
    ...
    private byte[] getDynamicFieldByteValue(Object value, Charset charset) {
        switch (dynamicFieldType) {
            // 如果是打包剩余的所有Byte
            case allRestByte:
                return ProtoPackageUtil.objString2Byte((String) value, charset);
            // 如果是文件metadata
            case metadata:
