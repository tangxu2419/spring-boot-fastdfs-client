...
import com.vcredit.framework.fastdfs.command.tracker.request.TrackerListStorageRequest;
 ...
 * @author tangxu
 */
public class ListStorageCommandInvoker extends AbstractTrackerCommandInvoker {

    public ListStorageCommandInvoker(TrackerCommand.ListStorage command) {
        this.command = command;
        super.request = new TrackerListStorageRequest(command.getGroupName(), command.getStorageIpAddr());
    }

    @Override
    protected BaseOperationResult parseContent(InputStream in, ProtoHead head, Charset charset) throws InvokeCommandException, IOException {
        byte[] content = ProtoPackageUtil.recvResponseBody(in, head.getContentLength());
        List<StorageState> list = packageList(StorageState.class, content, charset);
        return new StorageStateResult(list);
    }
